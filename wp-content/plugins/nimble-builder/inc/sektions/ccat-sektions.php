    ...
    switch( $input_id ) {
        case 'img-link-to' :
            $options = array(
                ...
                'img-file' => __('Image file', 'nimble-builder' ),
                'img-page' =>__('Image page', 'nimble-builder' )
            );
        break;
        ...
        case 'local_template' :
            $options = array(
                ...
                'nimble_template' => __('Nimble template','nimble-builder')
            );
        break;
            ...
            'item-inputs' => array(
                'content_type' => array(
                    ...
                    'notice_after' => sprintf(
                        __('Note : you can %1$s to replace your default theme template. Depending on your theme structure, it can allow you to display your sections in full-width mode.', 'nimble-builder'),
        ...
        'tmpl' => array(
            'item-inputs' => array(
                'module_id' => array(
                    ...
                    'title'       => __('Drag and drop modules in the previewed page', 'nimble-builder'),
                    'width-100'   => true,
                    'title_width' => 'width-100'
                )
            )
...
function sek_get_default_section_input_params() {
    return array(
        ...
        'title'       => __('Drag and drop sections in the previewed page', 'nimble-builder'),
        'width-100'   => true,
        'title_width' => 'width-100'
    );
        ...
        'tmpl' => array(
            'item-inputs' => array(
                'custom_anchor' => array(
                    ...
                    'default'     => '',
                    'notice_after' => __('Note : white spaces, numbers and special characters are not allowed when setting an anchor.', 'nimble-builder')
                ),
            )
        )//tmpl
    );
        ...
        'tmpl' => array(
            'item-inputs' => array(
                  'use-custom-breakpoint' => array(
                      'input_type'  => 'gutencheck',
                      'title'       => __('Use a custom breakpoint for the vertical reorganization of columns', 'nimble-builder'),
                      'default'     => 0,
                      'title_width' => 'width-80',
                      'input_width' => 'width-20',
                      'refresh_markup' => true,
        ...
        'tmpl' => array(
            'item-inputs' => array(
                'local_custom_css' => array(
                    ...
                    'notice_after' => __('This CSS code will be restricted to the currently previewed page only, and not applied site wide.', 'nimble-builder'),
                    'refresh_markup' => false,
                    'refresh_stylesheet' => true,
                )
            )
    ...
    return array(
        ...
        'tmpl' => array(
            'item-inputs' => array(
                'use-custom-breakpoint' => array(
                    'input_type'  => 'gutencheck',
                    'title'       => __('Use a global custom breakpoint for the vertical reorganization of columns', 'nimble-builder'),
                    'default'     => 0,
                    'title_width' => 'width-80',
                    'input_width' => 'width-20',
                    'notice_after' => __( 'This is the breakpoint under which columns are reorganized vertically. The default global breakpoint is 768px.', 'nimble-builder')
                ),
...
function sek_get_module_params_for_czr_simple_html_module() {
    return array(
        ...
        'starting_value' => array(
            'html_content' => sprintf('<pre>%1$s</pre>', __('html code goes here', 'nimble-builder') )
        ),
        'tmpl' => array(
            'item-inputs' => array(
                'html_content' => array(
                    ...
                    'default'     => sprintf('<pre>%1$s</pre>', __('html code goes here', 'nimble-builder') )
                )
            )
        ),
    ...
    return array(
        ...
        'tmpl' => array(
            'item-inputs' => array(
                'quote_text' => array(
                    'input_type'         => 'textarea',
                    'title'              => __( 'Quote text', 'nimble-builder' ),
                    'default'            => '',
                    'width-100'         => true,
                        ...
                        <div class="sek-column-inner <?php echo empty( $collection ) ? 'sek-empty-col' : ''; ?>">
                            <?php
                              if ( skp_is_customizing() && empty( $collection ) ) {
                                  ...
                                  $title = 'section' === $content_type ? __('Drag and drop a section here', 'nimble-builder' ) : __('Drag and drop a module here', 'nimble-builder' );
                                  ?>
...
abstract class Sek_Input_Abstract implements Sek_Input_Interface {
    ...
    public function get_value() {
        ...
        if ( skp_is_customizing() ) {
            ...
            switch( $field_name ) {
                case 'nimble_name' :
                    $value = __('John Doe', 'nimble-builder');
                break;
                case 'nimble_email' :
                    $value = __('john@doe.com', 'nimble-builder');
                break;
                case 'nimble_subject' :
        ...
        switch( $status ) {
            ...
            case 'aborted' :
                ...
                if ( false !== $this->invalid_field ) {
                    $submission_message = sprintf( __( '%1$s The following field is not well formed : <strong>%2$s</strong>.', 'nimble-builder' ), $submission_message, $this->invalid_field );
                }
            break;
        }
